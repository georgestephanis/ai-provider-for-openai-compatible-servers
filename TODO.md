...
- [ ] **`disable_thinking` sends non-standard params unconditionally** (`thinking`, `max_thinking_tokens`, `reasoning_effort`). Strict servers (e.g. vLLM in some configs, Apfel) reject unknown body fields with a 400. Send these selectively / make them configurable rather than all three at once.
