...
```
ai-provider-for-openai-compatible-servers.php
                                Bootstrap: provider registration, settings, REST route,
...
- **Two coding styles coexist.** `includes/` follows the AI Client SDK's PSR-12 style (4-space indent, camelCase methods, `declare(strict_types=1)`). `ai-provider-for-openai-compatible-servers.php` is procedural, namespaced WordPress code. PHPCS here runs PSR-12 plus the WordPress **security, i18n, and escaping** sniffs — not full WPCS formatting. Match the style of the file you're in.
...
- **Provider registration is defensive.** `ai-provider-for-openai-compatible-servers.php` no-ops if `WordPress\AiClient\AiClient` doesn't exist, so the plugin activates safely without the AI Client. Keep that guard.
- **All ten settings** live in the `connectors` settings group with `show_in_rest => true`; the JS saves them through `saveEntityRecord('root', 'site', …)` (the `/wp/v2/settings` endpoint), so any new setting must be registered in PHP *and* wired into the JS temp-state/save/remove handlers *and* added to `uninstall.php`'s cleanup list.
...
- **Provider detection is best-effort by design.** `detect_provider_type()` in `ai-provider-for-openai-compatible-servers.php` probes auxiliary, non-OpenAI-spec endpoints to guess which server is behind the configured base URL: Ollama's `GET {origin}/` (literal body `Ollama is running`), vLLM's `GET {origin}/version` (JSON `version` field), and LM Studio's `GET {origin}/api/v0/models` (JSON `data` array). None of these are guaranteed — a reverse proxy can mask any of them — so any probe failure (`WP_Error`, non-200, bad JSON) silently falls through to `null` rather than erroring; don't "fix" this into throwing or logging. The result rides along on the `test-connection` REST response as `provider`. Brand icons for a positive match live in `assets/images/providers/` (from LobeHub's MIT-licensed `lobe-icons`) and are resolved client-side in `src/connector-ui.js` via `new URL('../assets/images/providers/{slug}.svg', import.meta.url).href` — no PHP-to-JS data-passing needed since they're static build assets. Undetected servers keep the generic `assets/images/logo.svg`. The result is also cached in the `..._detected_provider` setting (written on Save/Remove, not on every test) so the connector card can seed `detectedProvider` during hydration and render the right name/icon on first paint instead of flashing the generic one while the background re-check runs; `src/connector-ui.js`'s `justHydratedRef` exists specifically to let that one seeded value survive the "reset test result on connection-param change" effect exactly once — it is not dead code.
