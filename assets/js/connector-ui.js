import { __experimentalRegisterConnector, __experimentalConnectorItem } from '@wordpress/connectors';

const { TextControl, Button, RadioControl, CheckboxControl, __experimentalHStack, __experimentalVStack } = window.wp.components;
...
const modelModeSettingName = 'connectors_ai_openai_compatible_servers_model_mode';
const modelsSettingName = 'connectors_ai_openai_compatible_servers_models';
const contextLengthSettingName = 'connectors_ai_openai_compatible_servers_context_length';
const disableThinkingSettingName = 'connectors_ai_openai_compatible_servers_disable_thinking';
const headersSettingName = 'connectors_ai_openai_compatible_servers_headers';
const supportsImagesSettingName = 'connectors_ai_openai_compatible_servers_supports_images';
const enableR1FormatSettingName = 'connectors_ai_openai_compatible_servers_enable_r1_format';

/**
 * Clean badge component to show connection status.
 */
const ConnectedBadge = () => el(
    'span',
    {
        style: {
            color: '#345b37',
 ...
 */
function OpenAiCompatibleServersConnector({ name, description, logo, plugin }) {
    ...
    const {
        ...
        return {
            ...
            currentDisableThinking: settings[disableThinkingSettingName] ?? false,
            currentHeaders: settings[headersSettingName] ?? '[]',
            currentSupportsImages: settings[supportsImagesSettingName] ?? false,
            currentEnableR1Format: settings[enableR1FormatSettingName] ?? false,
        };
    }, []);
    ...
    const [tempBaseUrl, setTempBaseUrl] = useState('http://localhost:11434/v1');
    const [tempModelMode, setTempModelMode] = useState('autodetect');
    const [tempSelectedCheckboxModels, setTempSelectedCheckboxModels] = useState([]);
    ...
    useEffect(() => {
        if (currentModels === undefined || availableModels.length === 0 || hasModelsInitialized) {
            return;
        }

        const savedModels = currentModels.split(',').map(m => m.trim()).filter(Boolean);
    ...
    useEffect(() => {
        ...
        if (tempModelMode === 'autodetect') {
            ...
            activeModels = availableModels.filter(m => tempSelectedCheckboxModels.includes(m.id));
        }
        ...
        activeModels.forEach(model => {
            if (model.max_model_len && model.max_model_len > detectedContext) {
        ...
        const visionKeywords = ['vl', 'vision', 'llava', 'clip', 'multimodal'];
        activeModels.forEach(model => {
            const searchStr = `${model.id} ${model.root || ''} ${model.name || ''}`.toLowerCase();
        ...
        activeModels.forEach(model => {
            const searchStr = `${model.id} ${model.root || ''} ${model.name || ''}`.toLowerCase();
            if (searchStr.includes('r1') || searchStr.includes('deepseek-r1')) {
                detectedR1 = true;
            }
        });
    ...
    }, [availableModels, tempModelMode, tempSelectedCheckboxModels]);
    ...
    const handleTestCredentials = async () => {
        ...
        try {
            ...
            if (data.success && Array.isArray(data.data)) {
                ...
                if (tempModelMode === 'manual' && tempSelectedCheckboxModels.length === 0 && !tempExtraManualModels) {
                ...
                setTestResult({
                    success: true,
                    message: sprintf(
                        __('Connection successful! Detected %d model(s).', 'ai-provider-for-openai-compatible-servers'),
                        data.data.length
                    )
                });
                ...
                createSuccessNotice(
                    ...
                    { id: 'openai-compatible-servers-test-success', type: 'snackbar' }
                );
            } else {
                throw new Error(data.message || __('Failed to connect to the server.', 'ai-provider-for-openai-compatible-servers'));
            }
        } catch (error) {
            ...
            setTestResult({
                success: false,
                message: sprintf(
                    __('Connection failed: %s', 'ai-provider-for-openai-compatible-servers'),
                    error.message || error
                )
            });
            createErrorNotice(
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const customList = tempExtraManualModels.split(',')
                .map(m => m.trim())
                .filter(Boolean);
            const modelsToSave = [...new Set([...tempSelectedCheckboxModels, ...customList])].join(', ');
            ...
            createSuccessNotice(
                __('OpenAI Compatible Servers connected successfully.', 'ai-provider-for-openai-compatible-servers'),
                { id: 'openai-compatible-servers-connect-success', type: 'snackbar' }
            );
        } catch (error) {
            console.error('Failed to save settings:', error);
            createErrorNotice(
                __('Failed to save settings. Please verify the server is running.', 'ai-provider-for-openai-compatible-servers'),
                { id: 'openai-compatible-servers-connect-error', type: 'snackbar' }
            );
        } finally {
            setIsBusy(false);
        }
    ...
    const handleRemove = async () => {
        ...
        try {
            ...
            createSuccessNotice(
                __('OpenAI Compatible Servers disconnected.', 'ai-provider-for-openai-compatible-servers'),
                { id: 'openai-compatible-servers-disconnect-success', type: 'snackbar' }
            );
        } catch (error) {
            console.error('Failed to disconnect:', error);
            createErrorNotice(
                __('Failed to disconnect.', 'ai-provider-for-openai-compatible-servers'),
                { id: 'openai-compatible-servers-disconnect-error', type: 'snackbar' }
            );
        } finally {
            setIsBusy(false);
        }
        ...
        __experimentalConnectorItem,
        {
            ...
            actionArea: el(
                ...
                el(
                    ...
                    isBusy ? __('Checking…') : isExpanded ? __('Cancel') : isConnected ? __('Edit') : __('Set up')
                )
            )
        },
        isExpanded && el(
            __experimentalVStack,
            { spacing: 4, className: 'connector-settings', style: { marginTop: '16px' } },
            el('style', null, `
            ...
            el(TextControl, {
                ...
                label: __('Base URL (URL & Port)', 'ai-provider-for-openai-compatible-servers'),
                value: tempBaseUrl,
                onChange: (val) => setTempBaseUrl(val),
                placeholder: 'http://localhost:11434/v1',
                disabled: isBusy,
                help: __('The base URL and port of your local OpenAI-compatible API server.', 'ai-provider-for-openai-compatible-servers')
            }),
            el(TextControl, {
                __next40pxDefaultSize: true,
                id: 'connectors_openai_compatible_servers_api_key_input',
                label: __('API Key', 'ai-provider-for-openai-compatible-servers'),
                value: tempApiKey,
                ...
                placeholder: 'local',
                disabled: isBusy,
                help: __('Optional. Dummy value used if local server does not require key.', 'ai-provider-for-openai-compatible-servers')
            }),

            el(
                __experimentalHStack,
                { alignment: 'flex-end', justify: 'space-between', spacing: 4, style: { marginBottom: '16px' } },
                el(RadioControl, {
                    label: __('Model Selection Mode', 'ai-provider-for-openai-compatible-servers'),
                    selected: tempModelMode,
                    options: [
                        { label: __('Autodetect all models from server', 'ai-provider-for-openai-compatible-servers'), value: 'autodetect' },
                        { label: __('Manually specify models', 'ai-provider-for-openai-compatible-servers'), value: 'manual' }
                    ],
                ...
                el(
                    ...
                    __('Test Credentials', 'ai-provider-for-openai-compatible-servers')
                )
            ),
            testResult && el(
                'div',
                {
                    style: {
                        ...
                        backgroundColor: testResult.success ? '#edf7ed' : '#fdeded',
                        padding: '8px 12px',
                        borderRadius: '4px',
                        marginBottom: '16px',
                        display: 'flex',
            ...
            tempModelMode === 'manual' && el(
                __experimentalVStack,
                { spacing: 4 },
                availableModels.length > 0 && el(
                    __experimentalVStack,
                    { spacing: 2 },
                    el('label', { style: { fontWeight: '600', fontSize: '13px', display: 'block', marginBottom: '4px' } }, __('Detected Models (select to enable)', 'ai-provider-for-openai-compatible-servers')),
                    availableModels.map(model => el(
                        CheckboxControl,
                        {
                            key: model.id,
                            label: model.id,
                            checked: tempSelectedCheckboxModels.includes(model.id),
                            onChange: (isChecked) => {
                                if (isChecked) {
                                    setTempSelectedCheckboxModels([...tempSelectedCheckboxModels, model.id]);
                                } else {
                                    setTempSelectedCheckboxModels(tempSelectedCheckboxModels.filter(m => m !== model.id));
                                }
                            }
                        }
                    ))
                ...
                el(TextControl, {
                    __next40pxDefaultSize: true,
                    label: __('Other Models (not listed above)', 'ai-provider-for-openai-compatible-servers'),
                    value: tempExtraManualModels,
                    onChange: (val) => setTempExtraManualModels(val),
                    placeholder: 'llama3.2, mistral',
                    disabled: isBusy,
                    help: __('Enter comma-separated names of any other models you want to enable that are not in the detected list above.', 'ai-provider-for-openai-compatible-servers')
                })
            ),
            el(
                'details',
                { style: { marginTop: '16px', borderTop: '1px solid #eee', paddingTop: '16px' } },
                el('summary', { style: { fontWeight: '600', cursor: 'pointer', marginBottom: '12px' } }, __('Advanced Settings', 'ai-provider-for-openai-compatible-servers')),
                el(
                    __experimentalVStack,
                    { spacing: 4 },
                    el(TextControl, {
                        type: 'number',
                        __next40pxDefaultSize: true,
                        label: __('Context Length (tokens)', 'ai-provider-for-openai-compatible-servers'),
                        value: tempContextLength || '',
                        onChange: (val) => setTempContextLength(val ? parseInt(val, 10) : 0),
                        placeholder: __('e.g., 8192', 'ai-provider-for-openai-compatible-servers'),
                        disabled: isBusy,
                        help: __('Optional. Configures num_ctx and max_tokens parameters to control server memory limits.', 'ai-provider-for-openai-compatible-servers')
                    }),
                    el(CheckboxControl, {
                        label: __('Disable thinking/reasoning blocks', 'ai-provider-for-openai-compatible-servers'),
                        checked: tempDisableThinking,
                        onChange: (val) => setTempDisableThinking(val),
                        disabled: isBusy,
                        help: __('Optimizes context usage and prevents long thinking preambles for reasoning models (e.g. Qwen 3.7 or DeepSeek-R1).', 'ai-provider-for-openai-compatible-servers')
                    }),
                    el(CheckboxControl, {
                        ...
                        disabled: isBusy,
                        help: __('Enable this option if your local models support multimodal image inputs (e.g., Llama 3.2 Vision).', 'ai-provider-for-openai-compatible-servers')
                    }),
                    el(CheckboxControl, {
                        label: __('Enable R1 Messages Format', 'ai-provider-for-openai-compatible-servers'),
                        checked: tempEnableR1Format,
                        onChange: (val) => setTempEnableR1Format(val),
                        disabled: isBusy,
                        help: __('Wraps system instructions within the first user message instead of a separate system message (necessary for DeepSeek-R1 compatibility).', 'ai-provider-for-openai-compatible-servers')
                    }),

                    // Custom HTTP Headers editor.
                    el(
                        __experimentalVStack,
                        { spacing: 2, style: { borderTop: '1px dashed #eee', paddingTop: '16px', marginTop: '8px' } },
                        el('label', { style: { fontWeight: '600', fontSize: '13px', display: 'block' } }, __('Custom HTTP Headers', 'ai-provider-for-openai-compatible-servers')),
                        tempHeaders.map((header, idx) => el(
                            __experimentalHStack,
                            { key: idx, spacing: 2 },
                            el(TextControl, {
                                __next40pxDefaultSize: true,
                                placeholder: __('Header Name', 'ai-provider-for-openai-compatible-servers'),
                                value: header.key,
                                onChange: (val) => {
                            ...
                            el(TextControl, {
                                __next40pxDefaultSize: true,
                                placeholder: __('Value', 'ai-provider-for-openai-compatible-servers'),
                                value: header.value,
                                onChange: (val) => {
                                    const next = [...tempHeaders];
                                    next[idx].value = val;
                            ...
                            el(
                                Button,
                                {
                                    variant: 'secondary',
                                    onClick: () => {
                                        const next = tempHeaders.filter((_, i) => i !== idx);
                                        setTempHeaders(next);
                                    }
                                },
                                __('Remove', 'ai-provider-for-openai-compatible-servers')
                            )
                        )),
                        el(
                            Button,
                            ...
                            __('Add Header', 'ai-provider-for-openai-compatible-servers')
                        )
                    )
                )
            ),
            el(
                ...
                el(
                    ...
                    __('Save', 'ai-provider-for-openai-compatible-servers')
                ),
                isConnected && el(
                    Button,
                    {
                    ...
                    __('Remove and replace', 'ai-provider-for-openai-compatible-servers')
                )
            )
        )
    );
